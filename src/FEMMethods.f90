...
contains
  ...
  call GlobalStiffness(GlobalStiffnessMatrix,Elms,numberOfElm,Nodes,errorFlag)
  ...
  !###############################
  subroutine GlobalStiffness(GlobalStiffnessMatrix, Elms,numberOfElm,Nodes,errorFlag)
    ...
    do i = 1, numberOfElm, 1
       ...
       ! Hvis GobalMartixConverter (GCM) er null Betyr det at verdien ikke skal være med videre pga. grensebetingerlser
       do j=1,DOF,1
          GMC(j)=(((elm%node1-1)*DOF) +j)* Nodes(elm%node1)%GDOF(j)
          GMC(j+3)=(((elm%node2-1)*DOF) +j) *  Nodes(elm%node2)%GDOF(j)
       end do
       if (pr_switch > 6)then
          print * ,''
          print *, Tag
